...
デモ用にNotionに以下のようなデータベースを用意します。Name(ノートのタイトル)とTagsをプロパティとして持ちます。この後のデモ用に`foo`というタイトルのノートを1つ入れておきました。
...
Notion APIを使うためには、以下の準備をする必要があります。[公式のドキュメント](https://developers.notion.com/docs/create-a-notion-integration)が詳しいので、細かい手順は割愛します。
...
この`title: {}`には、そのプロパティの何らかの情報が入るようになっているようです。例えば、`Tags`オブジェクトの`multi-select`プロパティには、選択肢の情報が入っていることがわかります。Notion APIの色々なオブジェクトがこのパターンに従うらしいです。
